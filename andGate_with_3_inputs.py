...
class Dataset():
	...
	def generate_data(self):
		self.data = np.array(np.random.random([self.size, 3]) >= 0.5, dtype=np.int32)
		self.label= np.array([ self.getLabel(d) for d in self.data])
...
class Network():
	...
	def train(self, iterationNo=1000):
		...
		print("After {0} times training, ".format(iterationNo), end='')
